...
module SegmentAPI
  class Analytics
    # Initializes a new instance of {SegmentAPI::Analytics::Client}, to which all
    # method calls are proxied.
    #
    # @param options includes options that are passed down to
    #   {SegmentAPI::Analytics::Client#initialize}
    # @option options [Boolean] :stub (false) If true, requests don't hit the
    #   server and are stubbed to be successful.
    def initialize(options = {})
      Request.stub = options[:stub] if options.has_key?(:stub)
      @client = SegmentAPI::Analytics::Client.new options
    end
